...
func Test_SaveFood_Invalid_Data(t *testing.T) {
	...
	for _, v := range samples {
		...
		r := gin.Default()
		r.POST("/comida", f.SaveFood)
		req, err := http.NewRequest(http.MethodPost, "/comida", bytes.NewBufferString(v.inputJSON))
		if err != nil {
...
func TestSaverFood_Success(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodPost, "/comida", &requestBody)
	if err != nil {
		t.Errorf("this is the error: %v\n", err)
	}
	r := gin.Default()
...
func TestGetAllFood_Success(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, "/comida", nil)
	if err != nil {
		t.Errorf("this is the error: %v\n", err)
	}
	r := gin.Default()
	r.GET("/comida", f.GetAllFood)
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestGetFoodAndCreator_Success(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodGet, "/comida/"+foodID, nil)
	if err != nil {
		t.Errorf("this is the error: %v\n", err)
	}
	r := gin.Default()
	r.GET("/comida/:com_id", f.GetFoodAndCreator)
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestUpdateFood_Success_With_File(t *testing.T) {
	...
	foodID := strconv.Itoa(1)
	req, err := http.NewRequest(http.MethodPut, "/comida/"+foodID, &requestBody)
	if err != nil {
		t.Errorf("this is the error: %v\n", err)
	}
	r := gin.Default()
	r.PUT("/comida/:com_id", f.UpdateFood)
	req.Header.Set("Authorization", tokenString)
	req.Header.Set("Content-Type", multipartWriter.FormDataContentType()) //this is important
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
...
func TestUpdateFood_Invalid_Data(t *testing.T) {
	...
	for _, v := range samples {
		...
		r := gin.Default()
		r.POST("/comida/:com_id", f.UpdateFood)
		req, err := http.NewRequest(http.MethodPost, "/comida/"+foodID, bytes.NewBufferString(v.inputJSON))
		if err != nil {
			t.Errorf("this is the error: %v\n", err)
		}
		req.Header.Set("Authorization", tokenString)
...
func TestDeleteFood_Success(t *testing.T) {
	...
	foodId := strconv.Itoa(1)
	req, err := http.NewRequest(http.MethodDelete, "/comida/"+foodId, nil)
	if err != nil {
		t.Errorf("this is the error: %v\n", err)
	}
	r := gin.Default()
	r.DELETE("/comida/:com_id", f.DeleteFood)
	req.Header.Set("Authorization", tokenString)
	rr := httptest.NewRecorder()
	r.ServeHTTP(rr, req)
