...
func TestSaveUser_Success(t *testing.T) {
	...
	req, err := http.NewRequest(http.MethodPost, "/usuarios", bytes.NewBufferString(inputJSON))
	if err != nil {
...
func Test_SaveUser_Invalidating_Data(t *testing.T) {
	...
	for _, v := range samples {
		...
		r := gin.Default()
		r.POST("/usuarios", s.SaveUser)
		req, err := http.NewRequest(http.MethodPost, "/usuarios", bytes.NewBufferString(v.inputJSON))
		if err != nil {
			t.Errorf("this is the error: %v\n", err)
		}
		rr := httptest.NewRecorder()
...
func TestGetUsers_Success(t *testing.T) {
	...
	r := gin.Default()
	r.GET("/usuarios", s.GetUsers)

	req, err := http.NewRequest(http.MethodGet, "/usuarios", nil)
	if err != nil {
		t.Errorf("this is the error: %v\n", err)
	}
	rr := httptest.NewRecorder()
...
func TestGetUser_Success(t *testing.T) {
	...
	r.GET("/usuarios/:usuar_id", s.GetUser)

	req, err := http.NewRequest(http.MethodGet, "/usuarios/"+userId, nil)
	if err != nil {
		t.Errorf("this is the error: %v\n", err)
	}
	rr := httptest.NewRecorder()
